Version 2 of Rideable Vehicles by Graham Nelson begins here.
...
Before getting off a rideable animal (called the steed), try
dismounting instead.
...
Before exiting:
	if the player is carried by a rideable animal, try dismounting instead;
	if the player is carried by a rideable vehicle, try dismounting instead.

Check mounting:
	if the player is carried by a rideable animal (called the steed),
		say "You are already riding [the steed]." instead;
	if the player is on a rideable vehicle (called the conveyance),
		say "You are already riding [the conveyance]." instead;
...
Carry out mounting (this is the standard mounting rule):
	surreptitiously move the actor to the noun.

Report mounting (this is the standard report mounting rule):
	say "You mount [the noun].";
	describe locale for the noun.
	
Check someone trying mounting (this is the no person mounting a mounted animal rule): 
	if the person asked is carried by a rideable animal (called the steed), 
		stop the action.

Check someone trying mounting (this is the no person mounting a mounted vehicle rule):
	if the person asked is on a rideable vehicle (called the conveyance), stop the action.

Check someone trying mounting (this is the no person mounting an unrideable thing rule):
	if the noun is not a rideable animal and the noun is not a rideable vehicle, 
		stop the action.

Check someone trying mounting (this is the person entering rule):
	abide by the can't enter something carried rule; 
	abide by the implicitly pass through other barriers rule. 
	
Carry out someone trying mounting (this is the standard person mounting rule): 
	surreptitiously move the actor to the noun.
	
Report someone trying mounting (this is the standard report person mounting rule): 
	say "[The person asked] mount[s] [the noun]." instead. 

Unsuccessful attempt by someone trying mounting (this is the mounting excuses rule):
	if the reason the action failed is the no person mounting a mounted animal rule,
		say "[The person asked] [is-are] already riding [the random rideable animal which carries the person asked].";
	if the reason the action failed is the no person mounting a mounted vehicle rule,
		say "[The person asked] [is-are] already riding [the random rideable vehicle which supports the person asked].";
	if the reason the action failed is the no person mounting an unrideable thing rule,
...
Check dismounting (this is the no dismounting when not mounted rule):
...
Carry out dismounting (this is the standard dismounting rule):
	if the player is carried by a rideable animal (called the steed),
...
Report dismounting (this is the standard report dismounting rule):
	say "You dismount [the noun].[line break][run paragraph on]";
	produce a room description with going spacing conventions.

Check someone trying dismounting (this is the no person dismounting when not mounted rule): 
	if the person asked is not carried by a rideable animal and the person asked is not on a rideable vehicle, 
		stop the action.
		
Carry out someone trying dismounting (this is the standard person dismounting rule): 
	if the person asked is carried by a rideable animal (called the steed), 
		change the noun to the steed; 
	if the person asked is on a rideable vehicle (called the conveyance), 
		change the noun to the conveyance; 
	let the former exterior be the holder of the noun;
	surreptitiously move the actor to the former exterior.
	
Report someone trying dismounting (this is the standard report person dismounting rule): 
	say "[The person asked] dismount[s] [the noun]."
	
Unsuccessful attempt by someone trying dismounting (this is the dismounting excuses rule):
	if the reason the action failed is the no person dismounting when not mounted rule,
