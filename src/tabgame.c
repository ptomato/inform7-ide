...
/* Run a story file that is not the current one being edited */
void run_game_file(struct story *thestory, gchar *filename) {
    ...
    /* Build command line */
    gchar **args;
    if(g_str_has_suffix(filename, ".ulx")
      || g_str_has_suffix(filename, ".gblorb")) {
        args = g_new(gchar *,4);
        args[0] = g_strdup("glulxe");
        args[1] = g_strdup("-w57");
    ...
    /* Save the PID and signal handler, but do not connect the "commit" signal,
    i.e. don't save the input in the skein */
    thestory->interp_running = TRUE;
    thestory->interp_process = vte_terminal_fork_command(term, args[0], args,
