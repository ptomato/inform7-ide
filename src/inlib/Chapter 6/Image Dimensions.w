...
@Purpose: These utility routines, contributed by Toby Nelson, look at the
headers of JPEG and PNG files to find the pixel dimensions of any images
supplied by the user for cover art and figures.

@-------------------------------------------------------------------------------

@p JPEG files.
The following either finds the pixel width and height of a given JPEG file
and returns |TRUE| or, if it can't read the file or doesn't recognise the
header as having JPEG format, returns |FALSE|.
...
beginning with an IHDR chunk and ending with an IEND chunk ({\it Portable
