...
|cblorb| as follows. Let |Path| be the path to the folder containing the Inform
...
output from |cblorb|, printed to |stdout| (of course the interface is free
to redirect this if it wants to). If |cblorb| printed a line in the form:
...
(...) If |cblorb| printed no such line, the interface should put up a Save As...
dialogue box, and invite the user to choose a destination.

@p |cblorb| at the command line.
When using |cblorb| as a command-line tool, it's probably convenient to
...
|cblorb| to use |\| instead of |/| as a filename separator.

The blorbfile filename is optional since |cblorb| does not always need to
...
|-project Whatever.inform|: tells |cblorb| to assume the usual settings for
...
@p Example blurb scripts.
This first script instructs |cblorb| to carry out its mission -- it makes a
...
These two lines tell |cblorb| to include the story file and the iFiction
record respectively.
...
@ And here's a very short script, which makes |cblorb| generate a solution
...
This time no blorb file is made. The opening line tells |cblorb| which Inform
...
tells |cblorb| where to put all of its output -- everything it makes. Only
the third line directly causes |cblorb| to do anything.
...
Template paths tell |cblorb| where to find templates. Any number of these
can be set -- including none at all, but if so then commands needing a
named template, like |website|, can't be used. |cblorb| looks for any
...
The command |css| tells |cblorb| that it is allowed to use CSS styles to
...
@ The previous examples all involved Inform projects, but |cblorb| can also
...
@p Specification of the Blurb language.
...
|cblorb| always does this, and does not support |storyfile| without
|include|.
...
Tells |cblorb| to look for associated resources, such as the Skein file,
within this Inform project.

	|release to| \bltoken{filename}

Tells |cblorb| that all of its output should go into this folder. (Well,
...
can be any number of template paths set, and |cblorb| checks them in order
...
|cblorb| doesn't just include it on a website: it generates multiple HTML
...
Here |cblorb| acts as no more than a file-copy utility; a verbatim copy of
...
This has an especially marked effect when |cblorb| is generating HTML
...
makes |cblorb| convert the Inform source text in the first filename into a
