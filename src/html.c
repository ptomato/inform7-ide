                 ...
                 gpointer data) 
{
    ...
    if(!g_file_get_contents(file, &buf, &length, &err)) {
    	...
    	/* Ignore an error in the Phrasebook.html generated by ni ver. 6E59 and 6E72 */
    	if(g_str_has_suffix(file, "paste.png")) {
    		g_error_free(err);
    		g_warning("Ignored error in Phrasebook.html");
    		g_free(file);
    		file = get_datafile_path_va("Documentation", "doc_images", "paste.png", NULL);
    		if(g_file_get_contents(file, &buf, &length, &err))
    			proceed = TRUE;
    	}
    	if(!proceed) {
